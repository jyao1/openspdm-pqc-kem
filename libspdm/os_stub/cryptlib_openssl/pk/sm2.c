  ...
  )
{
  ...
  ret_val = (boolean) EC_POINT_set_affine_coordinates(ec_group, ec_point, bn_x, bn_y, NULL);
  if (!ret_val) {
    goto done;
  }
  ...
  )
{
  ...
  *public_key_size = half_size * 2;
  ...
  ret_val = (boolean) EC_POINT_get_affine_coordinates(ec_group, ec_point, bn_x, bn_y, NULL);
  if (!ret_val) {
    goto done;
  }
  ...
  )
{
  ...
  if (sm2_context == NULL || peer_public == NULL || key_size == NULL || key == NULL) {
    return FALSE;
  }
  ...
  )
{
  ...
  *sig_size = half_size * 2;
  ...
  result = EVP_PKEY_CTX_set1_id(pkey_ctx, DEFAULT_SM2_ID, sizeof(DEFAULT_SM2_ID) - 1);
  if (result <= 0) {
    EVP_MD_CTX_free(ctx);
    EVP_PKEY_CTX_free(pkey_ctx);
    return FALSE;
  ...
  result = EVP_DigestSign(ctx, der_signature, &der_sig_size, message, size);
  if (result != 1) {
    EVP_MD_CTX_free(ctx);
    EVP_PKEY_CTX_free(pkey_ctx);
    return FALSE;
  ...
  )
{
  ...
  result = EVP_DigestVerify(ctx, der_signature, (uint32)der_sig_size, message, size);
  if (result != 1) {
    EVP_MD_CTX_free(ctx);
    EVP_PKEY_CTX_free(pkey_ctx);
    return FALSE;
