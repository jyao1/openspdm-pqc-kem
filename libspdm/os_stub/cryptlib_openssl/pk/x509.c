  ...
  )
{
  ...
  for (index = 0; ; index++) {
    ...
    //
    x509_cert = NULL;
    res = x509_construct_certificate (
               (const uint8 *) cert,
               cert_size,
               (uint8 **) &x509_cert
               );
  ...
  )
{
  ...
  ASN1_get_object ((const uint8 **)ptr, &obj_length, &obj_tag, &obj_class, (int32)(end - (*ptr)));
  ...
  )
{
  ...
  if (common_name == NULL) {
    ...
    *common_name_size = MIN ((uintn)length, *common_name_size - 1) + 1;
  ...
  )
{
  ...
  status = internal_x509_get_nid_name (x509_name, request_nid, common_name, common_name_size);
...
{
  return internal_x509_get_subject_nid_name(cert, cert_size, NID_commonName, common_name, common_name_size);
}
...
{
  return internal_x509_get_subject_nid_name (cert, cert_size, NID_organizationName, name_buffer, name_buffer_size);
}
  ...
  )
{
  ...
  *serial_number_size = (uintn)asn1_integer->length;
  if (serial_number != NULL) {
    copy_mem(serial_number, asn1_integer->data, *serial_number_size);
    status = RETURN_SUCCESS;
  }
...
{
  return internal_x509_get_issuer_nid_name(cert, cert_size, NID_commonName, common_name, common_name_size);
}
...
{
  return internal_x509_get_issuer_nid_name (cert, cert_size, NID_organizationName, name_buffer, name_buffer_size);
}
  ...
  )
{
  ...
  // Check input parameters.
  //
  if (cert == NULL || from_size == NULL || to_size == NULL || cert_size == 0) {
    return FALSE;
  }
  ...
  *from_size = f_size;
  if (from != NULL) {
    ...
    copy_mem(from + sizeof (ASN1_TIME), f_time->data, f_time->length);
  }
  ...
  )
{
  ...
  *date_time_size = d_size;
  if (date_time != NULL) {
    ...
    ((ASN1_TIME*)date_time)->data = (uint8 *)date_time + sizeof (ASN1_TIME);
    copy_mem((uint8 *)date_time + sizeof (ASN1_TIME), dt->data, dt->length);
  }
  status = RETURN_SUCCESS;
  ...
  )
{
  ...
  // Check input parameters.
  //
  if (cert == NULL || cert_size == 0 || oid == NULL || oid_size == 0 || extension_data_size == NULL) {
    return status;
  }
  ...
  for (i = 0; i < sk_X509_EXTENSION_num(extensions); i++) {
      ...
      if(oid_size == (uintn)asn1_obj->length && compare_mem (asn1_obj->data, oid, oid_size) == 0) {
        //
        // Extension Found
        //
        status = RETURN_SUCCESS;
  ...
  if (status == RETURN_SUCCESS) {
    ...
    if (oid != NULL) {
      copy_mem(extension_data, asn1_oct->data, asn1_oct->length);
    }
    *extension_data_size = asn1_oct->length;
    status = RETURN_SUCCESS;
  }
...
{
  return_status status;
  status = x509_get_extension_data (cert, cert_size, (uint8 *)m_oid_ext_key_usage, sizeof (m_oid_ext_key_usage), usage, usage_size);
  return status;
}
  ...
  )
{
  ...
  //
  if ((*rsa_context = RSAPublicKey_dup (EVP_PKEY_get0_RSA (pkey))) != NULL) {
    res = TRUE;
  }
  ...
  )
{
  ...
  //
  res = x509_construct_certificate (ca_cert, ca_cert_size, (uint8 **) &x509_ca_cert);
  ...
  )
{
  ...
  // Check input parameters.
  //
  if ((cert == NULL) || (tbs_cert == NULL) ||
      (tbs_cert_size == NULL) || (cert_size > INT_MAX)) {
    return FALSE;
  }
  ...
  temp   = cert;
  length = 0;
  ASN1_get_object (&temp, (long *)&length, (int *)&asn1_tag, (int *)&obj_class, (long)cert_size);
  ...
  *tbs_cert = (uint8 *)temp;
  ...
  ASN1_get_object (&temp, (long *)&length, (int *)&asn1_tag, (int *)&obj_class, (long)length);
  //
  ...
  )
{
  ...
  while (TRUE) {
    ...
    //
    verify_flag = x509_verify_cert(current_cert, current_cert_len, preceding_cert, preceding_cert_len);
