...
/// OID
///
static const uint8 m_oid_common_name[] = {
  0x55, 0x04, 0x03
};
static const uint8 m_oid_organization_name[] = {
  0x55, 0x04, 0x0A
};
static const uint8 m_oid_ext_key_usage[] = {
  0x55, 0x1D, 0x25
};
...
**/
boolean
x509_construct_certificate_stack (
  IN OUT  uint8  **x509_stack,
  ...
  )
{
...
return_status
internal_x509_get_nid_name (
  IN      mbedtls_x509_name     *name,
  IN      uint8         *oid,
  IN      uintn         oid_size,
  IN OUT  char8         *common_name,  OPTIONAL
  IN OUT  uintn         *common_name_size
  )
{
...
return_status
internal_x509_get_subject_nid_name (
  IN      const uint8   *cert,
  IN      uintn         cert_size,
  IN      uint8         *oid,
  IN      uintn         oid_size,
  OUT     char8         *common_name,  OPTIONAL
  IN OUT  uintn         *common_name_size
  )
{
  mbedtls_x509_crt crt;
  ...
  if (ret == 0) {
    name = &(crt.subject);
    status = internal_x509_get_nid_name(name, oid, oid_size, common_name, common_name_size);
  }
...
return_status
internal_x509_get_issuer_nid_name (
  IN      const uint8   *cert,
  IN      uintn         cert_size,
  IN      uint8         *oid,
  IN      uintn         oid_size,
...
{
  return internal_x509_get_subject_nid_name (cert, cert_size, (uint8 *)m_oid_common_name, sizeof (m_oid_common_name), common_name, common_name_size);
}
...
{
  return internal_x509_get_subject_nid_name (cert, cert_size, (uint8 *)m_oid_organization_name, sizeof (m_oid_organization_name), name_buffer, name_buffer_size);
}
...
x509_verify_cert (
  IN  const uint8  *cert,
  IN  uintn        cert_size,
  IN  const uint8  *ca_cert,
  IN  uintn        ca_cert_size
  )
{
  int32 ret;
  ...
  copy_mem(&profile, &mbedtls_x509_crt_profile_default, sizeof(mbedtls_x509_crt_profile));
  ...
  if (ret == 0) {
    ret = mbedtls_x509_crt_verify_with_profile(&end, &ca, NULL, &profile, NULL, &v_flag, NULL, NULL);
  }
  ...
  )
{
  ...
  do {
    ...
    ret = mbedtls_asn1_get_tag (&tmp_ptr, cert_chain + cert_chain_length, &asn1_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (ret != 0) {
      break;
    }
    ...
    if (x509_verify_cert (current_cert, current_cert_len, preceding_cert, preceding_cert_len) == FALSE) {
      verify_flag = FALSE;
      break;
    } else {
      verify_flag = TRUE;
  ...
  )
{
  ...
  //
  if ((cert_chain == NULL) || (cert == NULL) ||
      (cert_index < -1) || (cert_length == NULL)) {
    return FALSE;
  }
...
x509_get_serial_number (
  IN      const uint8   *cert,
  IN      uintn         cert_size,
  OUT     uint8         *serial_number,  OPTIONAL
  IN OUT  uintn         *serial_number_size
  )
{
  mbedtls_x509_crt crt;
  int32 ret;
...
{
  return internal_x509_get_issuer_nid_name (cert, cert_size, (uint8 *)m_oid_common_name, sizeof (m_oid_common_name), common_name, common_name_size);
}
...
**/
return_status
x509_get_issuer_orgnization_name (
  IN      const uint8   *cert,
  IN      uintn         cert_size,
  OUT     char8         *name_buffer,  OPTIONAL
  IN OUT  uintn         *name_buffer_size
...
{
  return internal_x509_get_issuer_nid_name (cert, cert_size, (uint8 *)m_oid_organization_name, sizeof (m_oid_organization_name), name_buffer, name_buffer_size);
}
...
static
return_status
internal_x509_find_extension_data (
  uint8 *start,
  uint8 *end,
  uint8 *oid,
  uintn oid_size,
  uint8 **find_extension_data,
  ...
  )
{
  ...
  while (TRUE) {
    ...
    */
    extension_ptr = ptr;
    ret = mbedtls_asn1_get_tag(&ptr, end, &obj_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (ret == 0) {
      header_len = (size_t)(ptr - extension_ptr);
      find_extension_len = obj_len;
      // Get Object Identifier
      ret = mbedtls_asn1_get_tag(&ptr, end, &obj_len, MBEDTLS_ASN1_OID);
    } else {
      break;
    }
    ...
    if (ret == 0 && compare_mem(ptr, oid, oid_size) == 0) {
      ...
      ret = mbedtls_asn1_get_tag(&ptr, end, &obj_len, MBEDTLS_ASN1_BOOLEAN);
      if (ret == 0) {
        ptr += obj_len;
      }
  ...
  )
{
  ...
  if (ret == 0) {
      status = internal_x509_find_extension_data(ptr, end, oid, oid_size, &ptr, &obj_len);
  }
  ...
  )
{
  ...
  if (ret == 0) {
    ...
    *to_size = t_size;
    if (to != NULL) {
      copy_mem(to, &(crt.valid_to), sizeof (mbedtls_x509_time));
    }
    status = TRUE;
  }
  ...
  )
{
  ...
  status = x509_get_extension_data((uint8 *)cert, cert_size, (uint8*)m_oid_ext_key_usage, sizeof (m_oid_ext_key_usage), usage, usage_size);
...
static
int32
internal_atoi(char8 *p_start, char8 *p_end) {
  char8 *p = p_start;
  int32 k = 0;
  while (p < p_end) {
    ///
  ...
  )
{
  ...
  if (compare_mem (date_time2, date_time1, sizeof (mbedtls_x509_time)) == 0) {
    return 0;
  }
  if(internal_x509_check_time ((mbedtls_x509_time*)date_time1, (mbedtls_x509_time*)date_time2) == 0) {
    return -1;
  } else {
    return 1;
  }
