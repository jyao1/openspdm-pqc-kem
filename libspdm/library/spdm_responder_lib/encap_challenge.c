...
**/
return_status
spdm_get_encap_request_challenge (
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_CHAL_CAP, 0)) {
  ...
  *encap_request_size = sizeof(spdm_challenge_request_t);
  ...
  // Cache data
  //
  status = spdm_append_message_mut_c (spdm_context, spdm_request, *encap_request_size);
  ...
  )
{
  ...
  if (spdm_response->header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_encap_error_response_main(spdm_context, &spdm_context->transcript.message_mut_c, spdm_context->encap_context.last_encap_request_size, spdm_response->header.param1);
    if (RETURN_ERROR(status)) {
      return status;
    }
  ...
  *(uint8 *)&auth_attribute = spdm_response->header.param1;
  if (spdm_context->encap_context.req_slot_id == 0xFF) {
    ...
    if (auth_attribute.slot_id != spdm_context->encap_context.req_slot_id) {
      return RETURN_DEVICE_ERROR;
    }
    if (spdm_response->header.param2 != (1 << spdm_context->encap_context.req_slot_id)) {
      return RETURN_DEVICE_ERROR;
    }
  }
  ...
  DEBUG((DEBUG_INFO, "Encap measurement_summary_hash (0x%x) - ", measurement_summary_hash_size));
  ...
  spdm_response_size = sizeof(spdm_challenge_auth_response_t) +
                     ...
                     opaque_length +
                     signature_size;
  status = spdm_append_message_mut_c (spdm_context, spdm_response, spdm_response_size - signature_size);
  if (RETURN_ERROR(status)) {
    return RETURN_SECURITY_VIOLATION;
  }
  ...
  result = spdm_verify_challenge_auth_signature (spdm_context, FALSE, signature, signature_size);
  if (!result) {
    spdm_context->encap_context.error_state = SPDM_STATUS_ERROR_CERTIFICATE_FAILURE;
    return RETURN_SECURITY_VIOLATION;
  }
