  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_KEY_EX_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_KEY_EX_CAP)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_KEY_EXCHANGE, response_size, response);
  ...
  if (spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_MUT_AUTH_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MUT_AUTH_CAP)) {
    if (spdm_context->encap_context.error_state != SPDM_STATUS_SUCCESS) {
      DEBUG((DEBUG_INFO, "spdm_get_response_key_exchange fail due to Mutual Auth fail\n"));
  ...
  opaque_data_length = *(uint16 *)((uint8 *)request + sizeof(spdm_key_exchange_request_t) + dhe_key_size + pqc_kem_public_key_size + pqc_kem_auth_cipher_text_size);
  if (request_size < sizeof(spdm_key_exchange_request_t) +
                    dhe_key_size +
                    pqc_kem_public_key_size +
                    pqc_kem_auth_cipher_text_size +
  ...
  ptr = (uint8 *)request + sizeof(spdm_key_exchange_request_t) + dhe_key_size + pqc_kem_public_key_size + pqc_kem_auth_cipher_text_size + sizeof(uint16);
  ...
  opaque_key_exchange_rsp_size = spdm_get_opaque_data_version_selection_data_size (spdm_context);
  ...
  *response_size = total_size;
  ...
  if (rsp_session_id == (INVALID_SESSION_ID & 0xFFFF)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_SESSION_LIMIT_EXCEEDED, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  ...
  if (spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_MUT_AUTH_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MUT_AUTH_CAP) &&
    ...
    spdm_response->mut_auth_requested = spdm_context->local_context.mut_auth_requested;
  }
  if (spdm_response->mut_auth_requested != 0) {
    spdm_init_mut_auth_encap_state (context, spdm_response->mut_auth_requested);
...
perf_start (PERF_ID_KEY_EX_KEM_ENCAP);
  if (dhe_key_size != 0) {
    ...
    internal_dump_hex ((uint8 *)request + sizeof(spdm_key_exchange_request_t), dhe_key_size);

    result = spdm_secured_message_dhe_compute_key (spdm_context->connection_info.algorithm.dhe_named_group, dhe_context, (uint8 *)request + sizeof(spdm_key_exchange_request_t), dhe_key_size, session_info->secured_message_context);
...
perf_start (PERF_ID_KEY_EX_KEM_AUTH_DECAP);
  if (use_pqc_kem_auth) {
    DEBUG((DEBUG_INFO, "Calc peer_key PQC_KEM_AUTH (0x%x):\n", pqc_kem_auth_cipher_text_size));
    internal_dump_hex ((uint8 *)request + sizeof(spdm_key_exchange_request_t) + dhe_key_size + pqc_kem_public_key_size, pqc_kem_auth_cipher_text_size);
...
perf_stop (PERF_ID_KEY_EX_KEM_AUTH_DECAP);

perf_start (PERF_ID_KEY_EX_REQ_KEM_AUTH_ENCAP);
  if (use_pqc_req_kem_auth) {
    result = spdm_pqc_requester_kem_auth_encap (
...
perf_stop (PERF_ID_KEY_EX_REQ_KEM_AUTH_ENCAP);
  ...
  *(uint16 *)ptr = (uint16)opaque_key_exchange_rsp_size;
  ptr += sizeof(uint16);
  status = spdm_build_opaque_data_version_selection_data (spdm_context, &opaque_key_exchange_rsp_size, ptr);
  ASSERT_RETURN_ERROR(status);
  ptr += opaque_key_exchange_rsp_size;

  spdm_context->connection_info.local_used_cert_chain_buffer = spdm_context->local_context.local_cert_chain_provision[slot_id];
  spdm_context->connection_info.local_used_cert_chain_buffer_size = spdm_context->local_context.local_cert_chain_provision_size[slot_id];

  status = spdm_append_message_k (session_info, spdm_response, (uintn)ptr - (uintn)spdm_response);
  if (RETURN_ERROR(status)) {
    spdm_free_session_id (spdm_context, session_id);
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
...
perf_stop (PERF_ID_KEY_EX_SIG_GEN);
  ...
  status = spdm_calculate_th1_hash (spdm_context, session_info, FALSE, th1_hash_data);
  if (RETURN_ERROR(status)) {
    spdm_free_session_id (spdm_context, session_id);
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
    result = spdm_generate_key_exchange_rsp_hmac (spdm_context, session_info, ptr);
    if (!result) {
      spdm_free_session_id (spdm_context, session_id);
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_KEY_EXCHANGE_RSP, response_size, response);
      return RETURN_SUCCESS;
  ...
  spdm_set_session_state (spdm_context, session_id, SPDM_SESSION_STATE_HANDSHAKING);
