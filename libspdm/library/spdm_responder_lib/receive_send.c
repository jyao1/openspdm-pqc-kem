...
**/
spdm_get_spdm_response_func
spdm_get_response_func_via_last_request (
  ...
  )
{
  ...
  return spdm_get_response_func_via_request_code (spdm_request->request_response_code);
}
  ...
  )
{
  ...
  spdm_context->last_spdm_request_size = sizeof(spdm_context->last_spdm_request);
  status = spdm_context->transport_decode_message (spdm_context, &message_session_id, is_app_message, TRUE, request_size, request, &spdm_context->last_spdm_request_size, spdm_context->last_spdm_request);
  ...
  if (spdm_context->last_spdm_request_size < sizeof(spdm_message_header_t)) {
  ...
  *session_id = message_session_id;
  ...
  if (message_session_id != NULL) {
    session_info = spdm_get_session_info_via_session_id (spdm_context, *message_session_id);
    if (session_info == NULL) {
      return RETURN_UNSUPPORTED;
    }
    spdm_context->last_spdm_request_session_id = *message_session_id;
  ...
  internal_dump_hex ((uint8 *)spdm_context->last_spdm_request, spdm_context->last_spdm_request_size);
  ...
  )
{
  ...
  for (index = 0; index < MAX_SPDM_SESSION_STATE_CALLBACK_NUM; index++) {
    if (spdm_context->spdm_session_state_callback[index] != 0) {
      ((spdm_session_state_callback_func)spdm_context->spdm_session_state_callback[index]) (spdm_context, session_id, session_state);
    }
  }
}
  ...
  )
{
  ...
  old_session_state = spdm_secured_message_get_session_state (session_info->secured_message_context);
  if (old_session_state != session_state) {
    spdm_secured_message_set_session_state (session_info->secured_message_context, session_state);
    spdm_trigger_session_state_callback (spdm_context, session_info->session_id, session_state);
  }
}
...
**/
void
spdm_trigger_connection_state_callback (
  IN     spdm_context_t      *spdm_context,
  ...
  )
{
  ...
  for (index = 0; index < MAX_SPDM_CONNECTION_STATE_CALLBACK_NUM; index++) {
    if (spdm_context->spdm_connection_state_callback[index] != 0) {
      ((spdm_connection_state_callback_func)spdm_context->spdm_connection_state_callback[index]) (spdm_context, connection_state);
    }
  }
}
  ...
  )
{
  if (spdm_context->connection_info.connection_state != connection_state) {
    ...
    spdm_trigger_connection_state_callback (spdm_context, connection_state);
  }
}
  ...
  )
{
  ...
  if (spdm_context->last_spdm_error.error_code != 0) {
    ...
    case SPDM_ERROR_CODE_DECRYPT_ERROR:
      ...
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_DECRYPT_ERROR, 0, &my_response_size, my_response);
      break;
    case SPDM_ERROR_CODE_INVALID_SESSION:
      // don't use session ID, because we dont know which right session ID should be used.
      spdm_generate_extended_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_SESSION, 0, sizeof(uint32), (void *)session_id, &my_response_size, my_response);
      session_id = NULL;
      break;
    default:
      ASSERT(FALSE);
    ...
    status = spdm_context->transport_encode_message (spdm_context, session_id, FALSE, FALSE, my_response_size, my_response, response_size, response);
    if (RETURN_ERROR(status)) {
      DEBUG((DEBUG_INFO, "transport_encode_message : %p\n", status));
      return status;
    }

    zero_mem (&spdm_context->last_spdm_error, sizeof(spdm_context->last_spdm_error));
    return RETURN_SUCCESS;
  }
  ...
  if (is_app_message || (get_response_func == NULL)) {
    if (spdm_context->get_response_func != 0) {
      status = ((spdm_get_response_func)spdm_context->get_response_func) (spdm_context, session_id, is_app_message, spdm_context->last_spdm_request_size, spdm_context->last_spdm_request, &my_response_size, my_response);
    } else {
      status = RETURN_NOT_FOUND;
    }
  }
  if (status != RETURN_SUCCESS) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, spdm_request->request_response_code, &my_response_size, my_response);
  }

  DEBUG((DEBUG_INFO, "SpdmSendResponse[%x] (0x%x): \n", (session_id != NULL) ? *session_id : 0, my_response_size));
  internal_dump_hex (my_response, my_response_size);

  status = spdm_context->transport_encode_message (spdm_context, session_id, is_app_message, FALSE, my_response_size, my_response, response_size, response);
  if (RETURN_ERROR(status)) {
    DEBUG((DEBUG_INFO, "transport_encode_message : %p\n", status));
    return status;
  }
  ...
  if (session_id != NULL) {
    ...
    case SPDM_FINISH_RSP:
      if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
        spdm_set_session_state (spdm_context, *session_id, SPDM_SESSION_STATE_ESTABLISHED);
      }
      break;
    ...
    case SPDM_END_SESSION_ACK:
      spdm_set_session_state (spdm_context, *session_id, SPDM_SESSION_STATE_NOT_STARTED);
    ...
    case SPDM_FINISH_RSP:
      if (spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
        spdm_set_session_state (spdm_context, spdm_context->latest_session_id, SPDM_SESSION_STATE_ESTABLISHED);
      }
      break;
    }
  }
...
spdm_register_get_response_func (
  IN  void                    *context,
  IN  spdm_get_response_func  get_response_func
  )
{
  spdm_context_t      *spdm_context;
  ...
  )
{
  ...
  for (index = 0; index < MAX_SPDM_SESSION_STATE_CALLBACK_NUM; index++) {
    if (spdm_context->spdm_session_state_callback[index] == 0) {
      spdm_context->spdm_session_state_callback[index] = (uintn)spdm_session_state_callback;
      return RETURN_SUCCESS;
    }
  }
  ASSERT(FALSE);
  ...
  )
{
  ...
  for (index = 0; index < MAX_SPDM_CONNECTION_STATE_CALLBACK_NUM; index++) {
    if (spdm_context->spdm_connection_state_callback[index] == 0) {
      spdm_context->spdm_connection_state_callback[index] = (uintn)spdm_connection_state_callback;
      return RETURN_SUCCESS;
    }
  }
  ASSERT(FALSE);
