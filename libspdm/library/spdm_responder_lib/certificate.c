  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP)) {
  ...
  // Cache
  //
  status = spdm_append_message_b (spdm_context, spdm_request, spdm_request_size);
  ...
  *response_size = sizeof(spdm_certificate_response_t) + length;
  ...
  // Cache
  //
  status = spdm_append_message_b (spdm_context, spdm_response, *response_size);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }

  spdm_set_connection_state (spdm_context, SPDM_CONNECTION_STATE_AFTER_CERTIFICATE);
