...
**/
boolean
spdm_create_measurement_signature (
  ...
  )
{
  ...
  ptr = (void *)((uint8 *)response_message + response_message_size - measurment_sig_size);
  
  spdm_get_random_number (SPDM_NONCE_SIZE, ptr);
  ptr += SPDM_NONCE_SIZE;

  *(uint16 *)ptr = (uint16)spdm_context->local_context.opaque_measurement_rsp_size;
  ptr += sizeof(uint16);
  copy_mem (ptr, spdm_context->local_context.opaque_measurement_rsp, spdm_context->local_context.opaque_measurement_rsp_size);
  ptr += spdm_context->local_context.opaque_measurement_rsp_size;

  status = spdm_append_message_m (spdm_context, response_message, response_message_size - signature_size);
  ...
  )
{
  ...
  measurment_no_sig_size = sizeof(uint16) +
                        spdm_context->local_context.opaque_measurement_rsp_size;
  ASSERT (response_message_size > measurment_no_sig_size);
  ptr = (void *)((uint8 *)response_message + response_message_size - measurment_no_sig_size);
...
**/
return_status
spdm_get_response_measurements (
  IN     void                 *context,
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_GET_MEASUREMENTS, response_size, response);
  ...
  if (!spdm_context->last_spdm_request_session_id_valid) {
    ...
    session_info = spdm_get_session_info_via_session_id (spdm_context, spdm_context->last_spdm_request_session_id);
  ...
  if (spdm_request->header.param1 == SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) {
    if (spdm_is_version_supported (spdm_context, SPDM_MESSAGE_VERSION_11)) {
      ...
      if (request_size < sizeof(spdm_get_measurements_request_t) - sizeof(spdm_request->SlotIDParam)) {
        spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
        return RETURN_SUCCESS;
      }
      request_size = sizeof(spdm_get_measurements_request_t) - sizeof(spdm_request->SlotIDParam);
    }
  } else {
  ...
  if ((spdm_request->header.param1 & SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) != 0) {
    if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP_SIG)) {
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
      return RETURN_SUCCESS;
    }
  }
  ...
  // Cache
  //
  status = spdm_append_message_m (spdm_context, spdm_request, request_size);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
    ...
    *response_size = spdm_response_size;
    ...
    if ((spdm_request->header.param1 & SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) != 0) {
      if (spdm_response->header.spdm_version >= SPDM_MESSAGE_VERSION_11) {
        ...
        if ((slot_id_param != 0xF) && (slot_id_param >= spdm_context->local_context.slot_count)) {
          spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
      ...
      status = spdm_create_measurement_signature (spdm_context, spdm_response, spdm_response_size);
      if (RETURN_ERROR(status)) {
        spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_GET_MEASUREMENTS, response_size, response);
        reset_managed_buffer (&spdm_context->transcript.message_m);
        return RETURN_SUCCESS;
      ...
      spdm_create_measurement_opaque (spdm_context, spdm_response, spdm_response_size);
    }
    break;
  ...
  case SPDM_GET_MEASUREMENTS_REQUEST_MEASUREMENT_OPERATION_ALL_MEASUREMENTS:
    ...
    for (index = 0; index < device_measurement_count; index++) {
      measurment_block_size = sizeof(spdm_measurement_block_dmtf_t) + cached_measurment_block->Measurement_block_dmtf_header.dmtf_spec_measurement_value_size;
    ...
    spdm_response_size = sizeof(spdm_measurements_response_t) + measurment_record_size;
    if ((spdm_request->header.param1 & SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) != 0) {
      spdm_response_size += measurment_sig_size;
    } else {
      spdm_response_size += measurment_no_sig_size;
    ...
    *response_size = spdm_response_size;
    ...
    spdm_write_uint24 (spdm_response->measurement_record_length, (uint32)measurment_record_size);

    measurment_block = (void *)(spdm_response + 1);
    cached_measurment_block = (void *)device_measurement;
    for (index = 0; index < device_measurement_count; index++) {
      measurment_block_size = sizeof(spdm_measurement_block_dmtf_t) + cached_measurment_block->Measurement_block_dmtf_header.dmtf_spec_measurement_value_size;
      copy_mem (measurment_block, cached_measurment_block, measurment_block_size);
      cached_measurment_block = (void *)((uintn)cached_measurment_block + measurment_block_size);
      measurment_block = (void *)((uintn)measurment_block + measurment_block_size);
    }
    ...
    if (spdm_request->header.param2 <= device_measurement_count) {
      ...
      *response_size = spdm_response_size;
      ...
      for (index = 0; index < device_measurement_count; index++) {
        ...
        if (index + 1 == spdm_request->header.param2) {
          copy_mem (measurment_block, cached_measurment_block, measurment_block_size);
          measurment_block->Measurement_block_common_header.index = 1; // always set to 1, since we only have 1 block.
          break;
        }
        cached_measurment_block = (void *)((uintn)cached_measurment_block + measurment_block_size);
      }
  ...
  if ((spdm_request->header.param1 & SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) != 0) {
    ...
    status = spdm_append_message_m (spdm_context, spdm_response, *response_size);
    if (RETURN_ERROR(status)) {
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
      reset_managed_buffer (&spdm_context->transcript.message_m);
      return RETURN_SUCCESS;
