  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
    ...
    if (!spdm_context->last_spdm_request_session_id_valid) {
      spdm_generate_error_response (context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
      return RETURN_SUCCESS;
    }
  } else {
  ...
  if (((session_info->mut_auth_requested == 0) && (spdm_request->header.param1 != 0)) ||
      ((session_info->mut_auth_requested != 0) && (spdm_request->header.param1 == 0)) ) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  ...
  status = spdm_append_message_f (session_info, request, sizeof(spdm_finish_request_t));
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  if (session_info->mut_auth_requested) {
    result = spdm_verify_finish_req_signature (spdm_context, session_info, (uint8 *)request + sizeof(spdm_finish_request_t), signature_size);
    if (!result) {
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_DECRYPT_ERROR, 0, response_size, response);
      return RETURN_SUCCESS;
    }
    status = spdm_append_message_f (session_info, (uint8 *)request + sizeof(spdm_finish_request_t), signature_size);
    if (RETURN_ERROR(status)) {
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
      return RETURN_SUCCESS;
    }
  ...
  result = spdm_verify_finish_req_hmac (spdm_context, session_info, (uint8 *)request + signature_size + sizeof(spdm_finish_request_t), hmac_size);
  if (!result) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_DECRYPT_ERROR, 0, response_size, response);
    return RETURN_SUCCESS;
  }

  status = spdm_append_message_f (session_info, (uint8 *)request + signature_size + sizeof(spdm_finish_request_t), hmac_size);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  ...
  *response_size = sizeof(spdm_finish_response_t) + hmac_size;
  ...
  status = spdm_append_message_f (session_info, spdm_response, sizeof(spdm_finish_response_t));
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  ...
  if (spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
    result = spdm_generate_finish_rsp_hmac (spdm_context, session_info, (uint8 *)spdm_response + sizeof(spdm_finish_request_t));
    if (!result) {
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_FINISH_RSP, response_size, response);
      return RETURN_SUCCESS;
    }

    status = spdm_append_message_f (session_info, (uint8 *)spdm_response + sizeof(spdm_finish_request_t), hmac_size);
    if (RETURN_ERROR(status)) {
      spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
      return RETURN_SUCCESS;
    }
  ...
  status = spdm_calculate_th2_hash (spdm_context, session_info, FALSE, th2_hash_data);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  status = spdm_generate_session_data_key (session_info->secured_message_context, th2_hash_data);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
