...
typedef struct {
  ...
  uint8                                request_op_code_sequence[MAX_ENCAP_REQUEST_OP_CODE_SEQUENCE_COUNT + 1];
...
typedef struct {
  ...
  //
  uintn                           spdm_connection_state_callback[MAX_SPDM_CONNECTION_STATE_CALLBACK_NUM];
...
**/
return_status
spdm_process_opaque_data_version_selection_data (
  IN     spdm_context_t  *spdm_context,
...
**/
return_status
spdm_process_opaque_data_supported_version_data (
  IN     spdm_context_t  *spdm_context,
  IN     uintn                data_in_size,
  IN     void                 *data_in
  );
