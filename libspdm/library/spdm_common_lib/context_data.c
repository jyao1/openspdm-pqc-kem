  ...
  )
{
  ...
  case SPDM_DATA_SPDM_VERSION:
    if (data_size > sizeof(spdm_version_number_t) * MAX_SPDM_VERSION_COUNT) {
      return RETURN_INVALID_PARAMETER;
    }
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.version.spdm_version_count = (uint8)(data_size / sizeof(spdm_version_number_t));
      copy_mem (
        spdm_context->connection_info.version.spdm_version,
        data,
        spdm_context->connection_info.version.spdm_version_count * sizeof(spdm_version_number_t)
        );
    } else {
      spdm_context->local_context.version.spdm_version_count = (uint8)(data_size / sizeof(spdm_version_number_t));
      copy_mem (
        spdm_context->local_context.version.spdm_version,
        data,
        spdm_context->local_context.version.spdm_version_count * sizeof(spdm_version_number_t)
        );
    }
    break;
  case SPDM_DATA_SECURED_MESSAGE_VERSION:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.secured_message_version.spdm_version_count = (uint8)(data_size / sizeof(spdm_version_number_t));
      copy_mem (
        spdm_context->connection_info.secured_message_version.spdm_version,
        data,
        spdm_context->connection_info.secured_message_version.spdm_version_count * sizeof(spdm_version_number_t)
        );
    } else {
      spdm_context->local_context.secured_message_version.spdm_version_count = (uint8)(data_size / sizeof(spdm_version_number_t));
      copy_mem (
  ...
  case SPDM_DATA_CAPABILITY_FLAGS:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.capability.flags = *(uint32 *)data;
    } else {
      spdm_context->local_context.capability.flags = *(uint32 *)data;
    }
    break;
  case SPDM_DATA_CAPABILITY_CT_EXPONENT:
    ...
    spdm_context->local_context.capability.ct_exponent = *(uint8 *)data;
    break;
  case SPDM_DATA_MEASUREMENT_SPEC:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.measurement_spec = *(uint8 *)data;
    } else {
      spdm_context->local_context.algorithm.measurement_spec = *(uint8 *)data;
    }
    break;
  case SPDM_DATA_MEASUREMENT_HASH_ALGO:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.measurement_hash_algo = *(uint32 *)data;
    } else {
      spdm_context->local_context.algorithm.measurement_hash_algo = *(uint32 *)data;
    }
    break;
  case SPDM_DATA_BASE_ASYM_ALGO:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.base_asym_algo = *(uint32 *)data;
    } else {
      spdm_context->local_context.algorithm.base_asym_algo = *(uint32 *)data;
    }
    break;
  case SPDM_DATA_BASE_HASH_ALGO:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.bash_hash_algo = *(uint32 *)data;
    } else {
      spdm_context->local_context.algorithm.bash_hash_algo = *(uint32 *)data;
    }
    break;
  case SPDM_DATA_DHE_NAME_GROUP:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.dhe_named_group = *(uint16 *)data;
    } else {
      spdm_context->local_context.algorithm.dhe_named_group = *(uint16 *)data;
    }
    break;
  case SPDM_DATA_AEAD_CIPHER_SUITE:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.aead_cipher_suite = *(uint16 *)data;
    } else {
      spdm_context->local_context.algorithm.aead_cipher_suite = *(uint16 *)data;
    }
    break;
  case SPDM_DATA_REQ_BASE_ASYM_ALG:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.req_base_asym_alg = *(uint16 *)data;
    } else {
      spdm_context->local_context.algorithm.req_base_asym_alg = *(uint16 *)data;
    }
    break;
  case SPDM_DATA_KEY_SCHEDULE:
    ...
    if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
      spdm_context->connection_info.algorithm.key_schedule = *(uint16 *)data;
    } else {
      spdm_context->local_context.algorithm.key_schedule = *(uint16 *)data;
    }
    break;
  case SPDM_DATA_CONNECTION_STATE:
    ...
    spdm_context->connection_info.connection_state = *(uint32 *)data;
    break;
  ...
  case SPDM_DATA_PEER_PUBLIC_ROOT_CERT_HASH:
    spdm_context->local_context.peer_root_cert_hash_provision_size = data_size;
    spdm_context->local_context.peer_root_cert_hash_provision = data;
    break;
  ...
  )
{
  ...
  case SPDM_DATA_SPDM_VERSION:
    ...
    target_data_size = spdm_context->connection_info.version.spdm_version_count * sizeof(spdm_version_number_t);
    target_data = spdm_context->connection_info.version.spdm_version;
    break;
  case SPDM_DATA_SECURED_MESSAGE_VERSION:
    if (parameter->location != SPDM_DATA_LOCATION_CONNECTION) {
      return RETURN_INVALID_PARAMETER;
    }
    target_data_size = spdm_context->connection_info.secured_message_version.spdm_version_count * sizeof(spdm_version_number_t);
    target_data = spdm_context->connection_info.secured_message_version.spdm_version;
    break;
  case SPDM_DATA_CAPABILITY_FLAGS:
  ...
  case SPDM_DATA_BASE_HASH_ALGO:
    ...
    target_data_size = sizeof(uint32);
    target_data = &spdm_context->connection_info.algorithm.bash_hash_algo;
    break;
  case SPDM_DATA_DHE_NAME_GROUP:
    if (parameter->location != SPDM_DATA_LOCATION_CONNECTION) {
      return RETURN_INVALID_PARAMETER;
  ...
  )
{
  ...
  spdm_context = context;
  return append_managed_buffer (&spdm_context->transcript.message_a, message, message_size);
}
  ...
  )
{
  ...
  spdm_context = context;
  return append_managed_buffer (&spdm_context->transcript.message_b, message, message_size);
}
  ...
  )
{
  ...
  spdm_context = context;
  return append_managed_buffer (&spdm_context->transcript.message_c, message, message_size);
}
  ...
  )
{
  ...
  spdm_context = context;
  return append_managed_buffer (&spdm_context->transcript.message_mut_b, message, message_size);
}
  ...
  )
{
  ...
  spdm_context = context;
  return append_managed_buffer (&spdm_context->transcript.message_mut_c, message, message_size);
}
  ...
  )
{
  ...
  spdm_context = context;
  return append_managed_buffer (&spdm_context->transcript.message_m, message, message_size);
}
...
**/
return_status
spdm_append_message_k (
  IN     void                                *session_info,
  IN     void                                *message,
  IN     uintn                               message_size
  )
{
  ...
  return append_managed_buffer (&spdm_session_info->session_transcript.message_k, message, message_size);
}
  ...
  )
{
  ...
  spdm_session_info = session_info;
  return append_managed_buffer (&spdm_session_info->session_transcript.message_f, message, message_size);
}
  ...
  )
{
  ...
  for (index = 0; index < spdm_context->connection_info.version.spdm_version_count; index++) {
    if ((major_version == spdm_context->connection_info.version.spdm_version[index].major_version) &&
        (minor_version == spdm_context->connection_info.version.spdm_version[index].minor_version) ) {
      return TRUE;
    }
  }
  return FALSE;
  ...
  )
{
  ...
  if (((requester_capabilities_flag == 0) || ((negotiated_requester_capabilities_flag & requester_capabilities_flag) != 0)) &&
      ((responder_capabilities_flag == 0) || ((negotiated_responder_capabilities_flag & responder_capabilities_flag) != 0)) ) {
    return TRUE;
  } else {
    return FALSE;
  }
...
spdm_register_device_io_func (
  IN     void                              *context,
  IN     spdm_device_send_message_func     send_message,
...
*/
void
spdm_get_last_spdm_error_struct (
  IN     void                      *context,
  ...
  )
{
  ...
  spdm_context = context;
  copy_mem (last_spdm_error, &spdm_context->last_spdm_error, sizeof(spdm_error_struct_t));
}
...
*/
void
spdm_set_last_spdm_error_struct (
  IN     void                      *context,
  ...
  )
{
  ...
  spdm_context = context;
  copy_mem (&spdm_context->last_spdm_error, last_spdm_error, sizeof(spdm_error_struct_t));
}
  ...
  )
{
  ...
  spdm_context->local_context.version.spdm_version[0].update_version_number = 0;
  ...
  spdm_context->local_context.version.spdm_version[1].update_version_number = 0;
  spdm_context->local_context.secured_message_version.spdm_version_count                   = 1;
  spdm_context->local_context.secured_message_version.spdm_version[0].major_version        = 1;
  ...
  for (index = 0; index < MAX_SPDM_SESSION_COUNT; index++) {
    spdm_context->session_info[index].secured_message_context = (void *)((uintn)secured_message_context + SecuredMessageContextSize * index);
    spdm_secured_message_init_context (spdm_context->session_info[index].secured_message_context);
  }
...
**/
uintn
spdm_get_context_size (
  void
  )
{
  return sizeof(spdm_context_t) + spdm_secured_message_get_context_size() * MAX_SPDM_SESSION_COUNT;
}
