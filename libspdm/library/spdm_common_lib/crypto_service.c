...
**/
boolean
spdm_get_peer_cert_chain_buffer (
  IN     void                     *context,
  ...
  )
{
  ...
  result = spdm_get_peer_cert_chain_buffer (spdm_context, cert_chain_data, cert_chain_data_size);
  if (!result) {
    return FALSE;
  }
  ...
  *cert_chain_data = (uint8 *)*cert_chain_data + sizeof(spdm_cert_chain_t) + hash_size;
...
**/
boolean
spdm_get_local_cert_chain_buffer (
  IN     void                     *context,
     OUT void                     **cert_chain_buffer,
     OUT uintn                    *cert_chain_buffer_size
  )
...
**/
boolean
spdm_get_local_cert_chain_data (
  IN     void                     *context,
     OUT void                     **cert_chain_data,
     OUT uintn                    *cert_chain_data_size
  )
{
  ...
  result = spdm_get_local_cert_chain_buffer (spdm_context, cert_chain_data, cert_chain_data_size);
  if (!result) {
    return FALSE;
  }
  ...
  )
{
  ...
  if (is_mut) {
    ...
    internal_dump_hex (get_managed_buffer(&spdm_context->transcript.message_mut_b), get_managed_buffer_size(&spdm_context->transcript.message_mut_b));
    status = append_managed_buffer (&m1m2, get_managed_buffer(&spdm_context->transcript.message_mut_b), get_managed_buffer_size(&spdm_context->transcript.message_mut_b));
    if (RETURN_ERROR(status)) {
      return FALSE;
    }

    DEBUG((DEBUG_INFO, "message_mut_c data :\n"));
    internal_dump_hex (get_managed_buffer(&spdm_context->transcript.message_mut_c), get_managed_buffer_size(&spdm_context->transcript.message_mut_c));
    status = append_managed_buffer (&m1m2, get_managed_buffer(&spdm_context->transcript.message_mut_c), get_managed_buffer_size(&spdm_context->transcript.message_mut_c));
    if (RETURN_ERROR(status)) {
      return FALSE;
    }

    // debug only
    spdm_hash_all (spdm_context->connection_info.algorithm.bash_hash_algo, get_managed_buffer(&m1m2), get_managed_buffer_size(&m1m2), hash_data);
    ...
    status = append_managed_buffer (&m1m2, get_managed_buffer(&spdm_context->transcript.message_a), get_managed_buffer_size(&spdm_context->transcript.message_a));
    if (RETURN_ERROR(status)) {
      return FALSE;
    }

    DEBUG((DEBUG_INFO, "message_b data :\n"));
    internal_dump_hex (get_managed_buffer(&spdm_context->transcript.message_b), get_managed_buffer_size(&spdm_context->transcript.message_b));
    status = append_managed_buffer (&m1m2, get_managed_buffer(&spdm_context->transcript.message_b), get_managed_buffer_size(&spdm_context->transcript.message_b));
    if (RETURN_ERROR(status)) {
      return FALSE;
    }

    DEBUG((DEBUG_INFO, "message_c data :\n"));
    internal_dump_hex (get_managed_buffer(&spdm_context->transcript.message_c), get_managed_buffer_size(&spdm_context->transcript.message_c));
    status = append_managed_buffer (&m1m2, get_managed_buffer(&spdm_context->transcript.message_c), get_managed_buffer_size(&spdm_context->transcript.message_c));
    if (RETURN_ERROR(status)) {
      return FALSE;
    }
  ...
  )
{
  ...
  internal_dump_hex (get_managed_buffer(&spdm_context->transcript.message_m), get_managed_buffer_size(&spdm_context->transcript.message_m));

  // debug only
  spdm_hash_all (spdm_context->connection_info.algorithm.bash_hash_algo, get_managed_buffer(&spdm_context->transcript.message_m), get_managed_buffer_size(&spdm_context->transcript.message_m), hash_data);
  DEBUG((DEBUG_INFO, "l1l2 hash - "));
  internal_dump_data (hash_data, hash_size);
  DEBUG((DEBUG_INFO, "\n"));

  *l1l2_buffer_size = get_managed_buffer_size(&spdm_context->transcript.message_m);
  copy_mem (l1l2_buffer, get_managed_buffer(&spdm_context->transcript.message_m), *l1l2_buffer_size);
...
**/
boolean
spdm_generate_cert_chain_hash (
  IN     spdm_context_t          *spdm_context,
  IN     uintn                        slot_id,
     OUT uint8                        *hash
  )
{
  if (spdm_context->local_context.slot_count == 0) {
    zero_mem (hash, spdm_get_hash_size (spdm_context->connection_info.algorithm.bash_hash_algo));
    return TRUE;
  }
...
**/
boolean
spdm_verify_peer_digests (
  IN spdm_context_t          *spdm_context,
  IN void                         *digest,
  IN uintn                        digest_size
  )
{
  uintn                                     hash_size;
  ...
  if ((cert_chain_buffer != NULL) && (cert_chain_buffer_size != 0)) {
    hash_size = spdm_get_hash_size (spdm_context->connection_info.algorithm.bash_hash_algo);
    spdm_hash_all (spdm_context->connection_info.algorithm.bash_hash_algo, cert_chain_buffer, cert_chain_buffer_size, cert_chain_buffer_hash);

    if (compare_mem (digest, cert_chain_buffer_hash, hash_size) != 0) {
      DEBUG((DEBUG_INFO, "!!! verify_peer_digests - FAIL !!!\n"));
      return FALSE;
    }
  }
...
**/
boolean
spdm_verify_peer_cert_chain_buffer (
  IN spdm_context_t          *spdm_context,
  ...
  )
{
  ...
  result = spdm_verify_certificate_chain_buffer (spdm_context->connection_info.algorithm.bash_hash_algo, cert_chain_buffer, cert_chain_buffer_size);
  if (!result) {
    return FALSE;
  }

  RootCertHash = spdm_context->local_context.peer_root_cert_hash_provision;
  RootCertHashSize = spdm_context->local_context.peer_root_cert_hash_provision_size;
  ...
  if ((RootCertHash != NULL) && (RootCertHashSize != 0)) {
    ...
    if (RootCertHashSize != hash_size) {
      DEBUG((DEBUG_INFO, "!!! verify_peer_cert_chain_buffer - FAIL (hash size mismatch) !!!\n"));
      return FALSE;
    }
    if (compare_mem ((uint8 *)cert_chain_buffer + sizeof(spdm_cert_chain_t), RootCertHash, hash_size) != 0) {
      DEBUG((DEBUG_INFO, "!!! verify_peer_cert_chain_buffer - FAIL (root hash mismatch) !!!\n"));
      return FALSE;
    }
  } else if ((cert_chain_data != NULL) && (cert_chain_data_size != 0)) {
    if (cert_chain_data_size != cert_chain_buffer_size) {
      DEBUG((DEBUG_INFO, "!!! verify_peer_cert_chain_buffer - FAIL !!!\n"));
      return FALSE;
    }
    if (compare_mem (cert_chain_buffer, cert_chain_data, cert_chain_buffer_size) != 0) {
      DEBUG((DEBUG_INFO, "!!! verify_peer_cert_chain_buffer - FAIL !!!\n"));
      return FALSE;
    }
  }
...
**/
boolean
spdm_generate_challenge_auth_signature (
  IN     spdm_context_t        *spdm_context,
  ...
  )
{
  ...
  if (is_requester) {
    use_pqc_kem_auth = !spdm_pqc_algo_is_zero (spdm_context->connection_info.algorithm.pqc_req_kem_auth_algo);
    if (spdm_context->local_context.pqc_public_key_mode == SPDM_DATA_PUBLIC_KEY_MODE_RAW) {
  ...
  )
{
  ...
  uintn                         m1m2_buffer_size;
  uint32                        hash_size;
  uint8                         mac_secret[MAX_HASH_SIZE];
  uint8                         mac_key[MAX_HASH_SIZE];
  uint8                         zero_filled_buffer[MAX_HASH_SIZE] = {0};
  ...
  result = spdm_hmac_all (
            spdm_context->connection_info.algorithm.bash_hash_algo,
            shared_key, shared_key_size,
            zero_filled_buffer, hash_size,
            mac_secret);
  ASSERT (result);
  result = spdm_hkdf_expand (
            spdm_context->connection_info.algorithm.bash_hash_algo,
            mac_secret, hash_size,
            "derive", sizeof("derive") - 1,
            mac_key, hash_size);
  ASSERT (result);
  DEBUG((DEBUG_INFO, "challenge gen MAC_KEY (0x%x):\n", hash_size));
  internal_dump_hex (mac_key, hash_size);
  result = spdm_hmac_all (
            spdm_context->connection_info.algorithm.bash_hash_algo,
            m1m2_buffer,
            m1m2_buffer_size,
...
**/
boolean
spdm_verify_certificate_chain_hash (
  IN spdm_context_t          *spdm_context,
  ...
  )
{
  ...
  result = spdm_get_peer_cert_chain_buffer (spdm_context, (void **)&cert_chain_buffer, &cert_chain_buffer_size);
  if (!result) {
    return FALSE;
  }
  ...
  if (hash_size != certificate_chain_hash_size) {
    DEBUG((DEBUG_INFO, "!!! verify_certificate_chain_hash - FAIL !!!\n"));
    return FALSE;
  }
  if (compare_mem (certificate_chain_hash, cert_chain_buffer_hash, certificate_chain_hash_size) != 0) {
    DEBUG((DEBUG_INFO, "!!! verify_certificate_chain_hash - FAIL !!!\n"));
    return FALSE;
  }
...
**/
boolean
spdm_verify_challenge_auth_signature (
  IN  spdm_context_t          *spdm_context,
  IN  boolean                      is_requester,
  IN  void                         *sign_data,
  ...
  )
{
  ...
  result = spdm_hkdf_expand (
            ...
            mac_key, hash_size);
  ASSERT (result);
  DEBUG((DEBUG_INFO, "challenge verify MAC_KEY (0x%x):\n", hash_size));
  internal_dump_hex (mac_key, hash_size);
  result = spdm_hmac_all (
            spdm_context->connection_info.algorithm.bash_hash_algo,
            m1m2_buffer,
  ...
  if (compare_mem (calc_hmac, hmac, hash_size) != 0) {
    DEBUG((DEBUG_INFO, "!!! verify_challenge_auth_hmac - FAIL !!!\n"));
    return FALSE;
  }
  DEBUG((DEBUG_INFO, "!!! verify_challenge_auth_hmac - PASS !!!\n"));
  return TRUE;
}
...
{
  if (!spdm_is_capabilities_flag_supported(spdm_context, is_requester, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP)) {
    return 0;
  }
  ...
  case SPDM_CHALLENGE_REQUEST_ALL_MEASUREMENTS_HASH:
    return spdm_get_hash_size (spdm_context->connection_info.algorithm.bash_hash_algo);
    break;
  }
  ...
  )
{
  ...
  case SPDM_CHALLENGE_REQUEST_ALL_MEASUREMENTS_HASH:
    ...
    for (index = 0; index < device_measurement_count; index++) {
      measurment_block_size = sizeof(spdm_measurement_block_common_header_t) + cached_measurment_block->Measurement_block_common_header.measurement_size;
      ASSERT (cached_measurment_block->Measurement_block_common_header.measurement_size == sizeof(spdm_measurement_block_dmtf_header_t) + cached_measurment_block->Measurement_block_dmtf_header.dmtf_spec_measurement_value_size);
      measurment_data_size += cached_measurment_block->Measurement_block_common_header.measurement_size;
      cached_measurment_block = (void *)((uintn)cached_measurment_block + measurment_block_size);
    }

    ASSERT (measurment_data_size <= MAX_SPDM_MEASUREMENT_RECORD_SIZE);
    ...
    for (index = 0; index < device_measurement_count; index++) {
      ...
      if (((measurement_summary_hash_type == SPDM_CHALLENGE_REQUEST_ALL_MEASUREMENTS_HASH) && 
           ((cached_measurment_block->Measurement_block_dmtf_header.dmtf_spec_measurement_value_type & SPDM_MEASUREMENT_BLOCK_MEASUREMENT_TYPE_MASK) < SPDM_MEASUREMENT_BLOCK_MEASUREMENT_TYPE_MEASUREMENT_MANIFEST)) ||
          ((cached_measurment_block->Measurement_block_dmtf_header.dmtf_spec_measurement_value_type & SPDM_MEASUREMENT_BLOCK_MEASUREMENT_TYPE_MASK) == SPDM_MEASUREMENT_BLOCK_MEASUREMENT_TYPE_IMMUTABLE_ROM)) {
        copy_mem (&measurement_data[measurment_data_size], &cached_measurment_block->Measurement_block_dmtf_header, cached_measurment_block->Measurement_block_common_header.measurement_size);
      }
      measurment_data_size += cached_measurment_block->Measurement_block_common_header.measurement_size;
      cached_measurment_block = (void *)((uintn)cached_measurment_block + measurment_block_size);
    }
    spdm_hash_all (spdm_context->connection_info.algorithm.bash_hash_algo, measurement_data, measurment_data_size, measurement_summary_hash);
    break;
  default:
    return FALSE;
    break;
...
**/
boolean
spdm_generate_measurement_signature (
  IN     spdm_context_t    *spdm_context,
     OUT uint8                  *signature
  )
{
...
**/
boolean
spdm_verify_measurement_signature (
  IN spdm_context_t          *spdm_context,
  IN void                         *sign_data,
  IN uintn                        sign_data_size
  )
