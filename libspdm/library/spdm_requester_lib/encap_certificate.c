...
**/
return_status
spdm_get_encap_response_certificate (
  IN     void                 *context,
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_CERT_CAP, 0)) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_GET_CERTIFICATE, response_size, response);
  ...
  // Cache
  //
  status = spdm_append_message_mut_b (spdm_context, spdm_request, request_size);
  ...
  *response_size = sizeof(spdm_certificate_response_t) + length;
  ...
  // Cache
  //
  status = spdm_append_message_mut_b (spdm_context, spdm_response, *response_size);
  if (RETURN_ERROR(status)) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
