  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_CHAL_CAP, 0)) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_CHALLENGE, response_size, response);
    return RETURN_SUCCESS;
  }

  if (request_size != sizeof(spdm_challenge_request_t)) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  ...
  // Cache
  //
  status = spdm_append_message_mut_c (spdm_context, spdm_request, request_size);
  ...
  *(uint16 *)ptr = (uint16)spdm_context->local_context.opaque_challenge_auth_rsp_size;
  ...
  // Calc Sign
  //
  status = spdm_append_message_mut_c (spdm_context, spdm_response, (uintn)ptr - (uintn)spdm_response);
  if (RETURN_ERROR(status)) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  result = spdm_generate_challenge_auth_signature (spdm_context, TRUE, ptr);
  if (!result) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_CHALLENGE_AUTH, response_size, response);
    return RETURN_SUCCESS;
  }
  ptr += signature_size;
