  ...
  )
{
  ...
  status = spdm_context->transport_encode_message (spdm_context, session_id, is_app_message, TRUE, request_size, request, &message_size, message);
...
perf_start (PERF_ID_REQUESTER);
  ...
  status = spdm_context->transport_decode_message (spdm_context, &message_session_id, &is_message_app_message, FALSE, message_size, message, response_size, response);

  if (session_id != NULL) {
    if (message_session_id == NULL) {
      DEBUG((DEBUG_INFO, "spdm_receive_spdm_response[%x] GetSessionId - NULL\n", (session_id != NULL) ? *session_id : 0x0));
      return RETURN_DEVICE_ERROR;
    }
    if (*message_session_id != *session_id) {
      DEBUG((DEBUG_INFO, "spdm_receive_spdm_response[%x] GetSessionId - %x\n", (session_id != NULL) ? *session_id : 0x0, *message_session_id));
      return RETURN_DEVICE_ERROR;
    }
  } else {
    if (message_session_id != NULL) {
  ...
  if ((is_app_message && !is_message_app_message) ||
    ...
    DEBUG((DEBUG_INFO, "spdm_receive_spdm_response[%x] app_message mismatch\n", (session_id != NULL) ? *session_id : 0x0));
    return RETURN_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  if ((session_id != NULL) &&
      spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
    ...
    if ((session_state == SPDM_SESSION_STATE_HANDSHAKING) && !session_info->use_psk) {
      session_id = NULL;
    }
  }

  return spdm_send_request (spdm_context, session_id, FALSE, request_size, request);
}
  ...
  )
{
  ...
  return spdm_receive_response (spdm_context, session_id, FALSE, response_size, response);
}