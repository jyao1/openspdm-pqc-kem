  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP)) {
  ...
  do {
    ...
    // Cache data
    //
    status = spdm_append_message_b (spdm_context, &spdm_request, sizeof(spdm_request));
    if (RETURN_ERROR(status)) {
    ...
    if (spdm_response.header.request_response_code == SPDM_ERROR) {
      status = spdm_handle_error_response_main(spdm_context, NULL, &spdm_context->transcript.message_b, sizeof(spdm_request), &spdm_response_size, &spdm_response, SPDM_GET_CERTIFICATE, SPDM_CERTIFICATE, sizeof(spdm_certificate_response_max_t));
      if (RETURN_ERROR(status)) {
        goto done;
      }
    ...
    // Cache data
    //
    status = spdm_append_message_b (spdm_context, &spdm_response, spdm_response_size);
    if (RETURN_ERROR(status)) {
      status = RETURN_SECURITY_VIOLATION;
      goto done;
    }
    ...
    internal_dump_hex (spdm_response.cert_chain, spdm_response.portion_length);

    status = append_managed_buffer (&certificate_chain_buffer, spdm_response.cert_chain, spdm_response.portion_length);
    if (RETURN_ERROR(status)) {
      status = RETURN_SECURITY_VIOLATION;
      goto done;
    }
  ...
  if (cert_chain_size != NULL) {
    if (*cert_chain_size < get_managed_buffer_size(&certificate_chain_buffer)) {
      *cert_chain_size = get_managed_buffer_size(&certificate_chain_buffer);
...
{
  return spdm_get_certificate_choose_length(context, slot_id, MAX_SPDM_CERT_CHAIN_BLOCK_LEN, cert_chain_size, cert_chain);
}
...
**/
return_status
spdm_get_certificate_choose_length (
  IN     void                 *context,
  IN     uint8                slot_id,
  IN     uint16               length,
  IN OUT uintn                *cert_chain_size,
  ...
  )
{
  ...
  do {
    status = try_spdm_get_certificate(spdm_context, slot_id, length, cert_chain_size, cert_chain);
