  ...
  )
{
  ...
  if (session_info->mut_auth_requested != 0) {
    if ((req_slot_id_param >= spdm_context->local_context.slot_count) && (req_slot_id_param != 0xFF)) {
  ...
  if (session_info->mut_auth_requested) {
    spdm_context->connection_info.local_used_cert_chain_buffer = spdm_context->local_context.local_cert_chain_provision[req_slot_id_param];
    spdm_context->connection_info.local_used_cert_chain_buffer_size = spdm_context->local_context.local_cert_chain_provision_size[req_slot_id_param];
  }
  ...
  status = spdm_append_message_f (session_info, (uint8 *)&spdm_request, sizeof(spdm_finish_request_t));
  if (RETURN_ERROR(status)) {
    return RETURN_SECURITY_VIOLATION;
  }
  if (session_info->mut_auth_requested) {
    result = spdm_generate_finish_req_signature (spdm_context, session_info, ptr);
    if (!result) {
      return RETURN_SECURITY_VIOLATION;
    }
    status = spdm_append_message_f (session_info, ptr, signature_size);
    if (RETURN_ERROR(status)) {
      return RETURN_SECURITY_VIOLATION;
    }
    ptr += signature_size;
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, &session_id, &session_info->session_transcript.message_f, spdm_request_size, &spdm_response_size, &spdm_response, SPDM_FINISH, SPDM_FINISH_RSP, sizeof(spdm_finish_response_mine_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
  ...
  if (spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HANDSHAKE_IN_THE_CLEAR_CAP)) {
    ...
    result = spdm_verify_finish_rsp_hmac (spdm_context, session_info, spdm_response.verify_data, hmac_size);
    if (!result) {
      return RETURN_SECURITY_VIOLATION;
    }

    status = spdm_append_message_f (session_info, (uint8 *)&spdm_response + sizeof(spdm_finish_response_t), hmac_size);
    if (RETURN_ERROR(status)) {
      return RETURN_SECURITY_VIOLATION;
    }
  }
  ...
  status = spdm_calculate_th2_hash (spdm_context, session_info, TRUE, th2_hash_data);
  if (RETURN_ERROR(status)) {
    return RETURN_SECURITY_VIOLATION;
  }
  ...
  )
{
  ...
  do {
    status = try_spdm_send_receive_finish(spdm_context, session_id, req_slot_id_param);
