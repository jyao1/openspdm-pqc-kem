...
typedef struct {
  ...
  uint8                measurement_record[(sizeof(spdm_measurement_block_dmtf_t) + MAX_HASH_SIZE) * MAX_SPDM_MEASUREMENT_BLOCK_COUNT];
  ...
  )
{
  ...
  if (session_id == NULL) {
    if (spdm_context->connection_info.connection_state < SPDM_CONNECTION_STATE_AUTHENTICATED) {
      return RETURN_UNSUPPORTED;
    }
  } else {
    ...
    session_state = spdm_secured_message_get_session_state (session_info->secured_message_context);
  ...
  if ((slot_id_param == 0xF) && (spdm_context->local_context.peer_cert_chain_provision_size == 0)) {
    return RETURN_INVALID_PARAMETER;
  }

  spdm_context->error_state = SPDM_STATUS_ERROR_DEVICE_NO_CAPABILITIES;

  if (spdm_is_capabilities_flag_supported(spdm_context, TRUE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP_NO_SIG) &&
      (request_attribute != 0)) {
    return RETURN_INVALID_PARAMETER;
  }
  ...
  if (request_attribute == SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) {
    if (spdm_is_version_supported (spdm_context, SPDM_MESSAGE_VERSION_11)) {
      ...
      spdm_request_size = sizeof(spdm_request) - sizeof(spdm_request.SlotIDParam);
    }
  ...
  // Cache data
  //
  status = spdm_append_message_m (spdm_context, &spdm_request, spdm_request_size);
  if (RETURN_ERROR(status)) {
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, session_id, &spdm_context->transcript.message_m, spdm_request_size, &spdm_response_size, &spdm_response, SPDM_GET_MEASUREMENTS, SPDM_MEASUREMENTS, sizeof(spdm_measurements_response_max_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
  ...
  measurement_record_data_length = spdm_read_uint24 (spdm_response.measurement_record_length);
  if (measurement_operation == SPDM_GET_MEASUREMENTS_REQUEST_MEASUREMENT_OPERATION_TOTAL_NUMBER_OF_MEASUREMENTS) {
    ...
    if (spdm_response_size < sizeof(spdm_measurements_response_t) + measurement_record_data_length) {
      return RETURN_DEVICE_ERROR;
    }
  ...
  if (request_attribute == SPDM_GET_MEASUREMENTS_REQUEST_ATTRIBUTES_GENERATE_SIGNATURE) {
    ...
    if (spdm_is_version_supported (spdm_context, SPDM_MESSAGE_VERSION_11) && spdm_response.header.param2 != slot_id_param) {
      reset_managed_buffer (&spdm_context->transcript.message_m);
      return RETURN_SECURITY_VIOLATION;
    }
    ...
    spdm_response_size = sizeof(spdm_measurements_response_t) +
                       ...
                       opaque_length +
                       signature_size;
    status = spdm_append_message_m (spdm_context, &spdm_response, spdm_response_size - signature_size);
    if (RETURN_ERROR(status)) {
      reset_managed_buffer (&spdm_context->transcript.message_m);
      return RETURN_SECURITY_VIOLATION;
    }
    ...
    result = spdm_verify_measurement_signature (spdm_context, signature, signature_size);
    if (!result) {
      spdm_context->error_state = SPDM_STATUS_ERROR_MEASUREMENT_AUTH_FAILURE;
      reset_managed_buffer (&spdm_context->transcript.message_m);
      return RETURN_SECURITY_VIOLATION;
    }
    ...
    spdm_response_size = sizeof(spdm_measurements_response_t) +
                       ...
                       sizeof(uint16) +
                       opaque_length;
    status = spdm_append_message_m (spdm_context, &spdm_response, spdm_response_size);
    if (RETURN_ERROR(status)) {
      reset_managed_buffer (&spdm_context->transcript.message_m);
      return RETURN_SECURITY_VIOLATION;
    }
  ...
  if (measurement_operation == SPDM_GET_MEASUREMENTS_REQUEST_MEASUREMENT_OPERATION_TOTAL_NUMBER_OF_MEASUREMENTS) {
    ...
    *number_of_blocks = spdm_response.number_of_blocks;
    ...
    while (measurement_block_size < measurement_record_data_length) {
      measurement_block_header = (spdm_measurement_block_common_header_t*) &measurement_record_data[measurement_block_size];
      if (measurement_block_header->measurement_size > measurement_record_data_length - ((uint8 *)measurement_block_header - (uint8 *)measurement_record_data)) {
        return RETURN_DEVICE_ERROR;
      }
      if (measurement_block_header->measurement_specification == 0 || (measurement_block_header->measurement_specification & (measurement_block_header->measurement_specification-1))) {
        return RETURN_DEVICE_ERROR;
      }
      if (measurement_block_header->measurement_specification != spdm_context->connection_info.algorithm.measurement_spec) {
        return RETURN_DEVICE_ERROR;
      }
      if (measurement_block_header->index == 0 || measurement_block_header->index > *number_of_blocks) {
        return RETURN_DEVICE_ERROR;
      }
      ...
      measurement_block_size = (uint32) (measurement_block_size + sizeof(spdm_measurement_block_common_header_t) + measurement_block_header->measurement_size);
    }

    *measurement_record_length = measurement_record_data_length;
    copy_mem (measurement_record, measurement_record_data, measurement_record_data_length);
  }
  ...
  )
{
  ...
  do {
    status = try_spdm_get_measurement(spdm_context, session_id, request_attribute, measurement_operation, slot_id_param, number_of_blocks, measurement_record_length, measurement_record);
