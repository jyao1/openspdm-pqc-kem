...
**/
hash_all_func
get_spdm_measurement_hash_func (
  IN   uint32                       measurement_hash_algo
  )
{
  switch (measurement_hash_algo) {
...
get_spdm_asym_get_public_key_from_x509 (
  IN   uint32                       base_asym_algo
  )
{
  switch (base_asym_algo) {
...
**/
boolean
spdm_asym_get_public_key_from_x509 (
  IN   uint32                       base_asym_algo,
  IN   const uint8                  *cert,
  IN   uintn                        cert_size,
  OUT  void                         **context
  )
{
  ...
  get_public_key_from_x509_function = get_spdm_asym_get_public_key_from_x509 (base_asym_algo);
  ...
  )
{
  ...
  if (need_hash) {
    ...
    result = spdm_hash_all (bash_hash_algo, message, message_size, message_hash);
    if (!result) {
      return FALSE;
    }
    return verify_function (context, hash_nid, message_hash, hash_size, signature, sig_size);
  } else {
    return verify_function (context, hash_nid, message, message_size, signature, sig_size);
  }
}
...
get_spdm_asym_get_private_key_from_pem (
  IN   uint32                       base_asym_algo
  )
{
  switch (base_asym_algo) {
...
**/
boolean
spdm_asym_get_private_key_from_pem (
  IN   uint32                       base_asym_algo,
  ...
  )
{
  ...
  asym_get_private_key_from_pem = get_spdm_asym_get_private_key_from_pem (base_asym_algo);
  ...
  )
{
  ...
  if (need_hash) {
    ...
    return asym_sign (context, hash_nid, message_hash, hash_size, signature, sig_size);
  } else {
    return asym_sign (context, hash_nid, message, message_size, signature, sig_size);
  }
}
...
**/
asym_get_public_key_from_x509_func
get_spdm_req_asym_get_public_key_from_x509 (
  IN   uint16                       req_base_asym_alg
  )
{
...
{
  asym_get_public_key_from_x509_func   get_public_key_from_x509_function;
  get_public_key_from_x509_function = get_spdm_req_asym_get_public_key_from_x509 (req_base_asym_alg);
  if (get_public_key_from_x509_function == NULL) {
    return FALSE;
  }
  return get_public_key_from_x509_function (cert, cert_size, context);
...
**/
asym_get_private_key_from_pem_func
get_spdm_req_asym_get_private_key_from_pem (
  IN   uint16                       req_base_asym_alg
  )
{
...
**/
boolean
spdm_req_asym_get_private_key_from_pem (
  IN   uint16                       req_base_asym_alg,
  IN   const uint8                  *pem_data,
  IN   uintn                        pem_size,
  IN   const char8                  *password,
...
{
  asym_get_private_key_from_pem_func   asym_get_private_key_from_pem;
  asym_get_private_key_from_pem = get_spdm_req_asym_get_private_key_from_pem (req_base_asym_alg);
  if (asym_get_private_key_from_pem == NULL) {
    return FALSE;
  }
  return asym_get_private_key_from_pem (pem_data, pem_size, password, context);
...
**/
dhe_generate_key_func
get_spdm_dhe_generate_key (
  IN   uint16                       dhe_named_group
  )
{
  switch (dhe_named_group) {
...
**/
dhe_compute_key_func
get_spdm_dhe_compute_key (
  IN      uint16                       dhe_named_group
  )
{
  switch (dhe_named_group) {
  ...
  )
{
  ...
  return ComputeKeyFunction (context, peer_public, peer_public_size, key, key_size);
}
  ...
  )
{
  ...
  return aead_enc_function (key, key_size, iv, iv_size, a_data, a_data_size, data_in, data_in_size, tag_out, tag_size, data_out, data_out_size);
}
  ...
  )
{
  ...
  return aead_dec_function (key, key_size, iv, iv_size, a_data, a_data_size, data_in, data_in_size, tag, tag_size, data_out, data_out_size);
}
...
static
boolean
internal_spdm_x509_date_time_check (
  IN uint8 *from,
  IN uintn from_size,
  IN uint8 *to,
  IN uintn to_size
  ...
  )
{
  ...
  // 6. Validaity
  status = x509_get_validity (cert, cert_size, end_cert_from, &end_cert_from_len, end_cert_to, &end_cert_to_len);
  if (!status) {
    goto cleanup;
  }

  status = internal_spdm_x509_date_time_check(end_cert_from, end_cert_from_len, end_cert_to, end_cert_to_len);
  if (!status) {
    goto cleanup;
  }
...
static const uint8 m_oid_subject_alt_name[] = {
  0x55, 0x1D, 0x11
};
...
**/
return_status
spdm_get_dmtf_subject_alt_name_from_bytes (
  IN      const uint8   *buffer,
  IN      intn          len,
  OUT     char8         *name_buffer,  OPTIONAL
  IN OUT  uintn         *name_buffer_size,
  OUT     uint8         *oid,         OPTIONAL
  IN OUT  uintn         *oid_size
  )
{
  uint8       *ptr;
  int32       length;
...
**/
return_status
spdm_get_dmtf_subject_alt_name (
  IN      const uint8   *cert,
  IN      intn          cert_size,
  OUT     char8         *name_buffer,  OPTIONAL
  IN OUT  uintn         *name_buffer_size,
  ...
  )
{
  ...
  status = x509_get_extension_data(cert, cert_size, (uint8 *)m_oid_subject_alt_name, sizeof (m_oid_subject_alt_name), NULL, &extension_data_size);
  ...
  status = x509_get_extension_data(cert, cert_size, (uint8 *)m_oid_subject_alt_name, sizeof (m_oid_subject_alt_name), (uint8 *)name_buffer, name_buffer_size);
  if (RETURN_ERROR(status)) {
    return status;
  }

  return spdm_get_dmtf_subject_alt_name_from_bytes((const uint8 *)name_buffer, *name_buffer_size, name_buffer, name_buffer_size, oid, oid_size);
}
  ...
  )
{
  ...
  if (!x509_get_cert_from_cert_chain (cert_chain_data, cert_chain_data_size, 0, &root_cert_buffer, &root_cert_buffer_size)) {
    DEBUG((DEBUG_INFO, "!!! VerifyCertificateChainData - FAIL (get root certificate failed)!!!\n"));
    return FALSE;
  }

  if (!x509_verify_cert_chain (root_cert_buffer, root_cert_buffer_size, cert_chain_data, cert_chain_data_size)) {
    DEBUG((DEBUG_INFO, "!!! VerifyCertificateChainData - FAIL (cert chain verify failed)!!!\n"));
    return FALSE;
  }

  if (!x509_get_cert_from_cert_chain (cert_chain_data, cert_chain_data_size, -1, &leaf_cert_buffer, &leaf_cert_buffer_size)) {
    DEBUG((DEBUG_INFO, "!!! VerifyCertificateChainData - FAIL (get leaf certificate failed)!!!\n"));
    return FALSE;
  }

  if(!spdm_x509_certificate_check (leaf_cert_buffer, leaf_cert_buffer_size)) {
...
**/
boolean
spdm_verify_certificate_chain_buffer (
  IN uint32                       bash_hash_algo,
  ...
  )
{
  ...
  cert_chain_data = (uint8 *)cert_chain_buffer + sizeof(spdm_cert_chain_t) + hash_size;
  ...
  spdm_hash_all (bash_hash_algo, root_cert_buffer, root_cert_buffer_size, calc_root_cert_hash);
  if (compare_mem ((uint8 *)cert_chain_buffer + sizeof(spdm_cert_chain_t), calc_root_cert_hash, hash_size) != 0) {
