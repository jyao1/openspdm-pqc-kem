...
**/
uintn
spdm_secured_message_get_context_size (
  void
  )
{
  ...
  )
{
  ...
  zero_mem (secured_message_context, sizeof(spdm_secured_message_context_t));
...
*/
void
spdm_secured_message_set_use_psk (
  IN void                         *spdm_secured_message_context,
  IN boolean                      use_psk
  )
{
  ...
  )
{
  ...
  secured_message_context->hash_size      = spdm_get_hash_size (secured_message_context->bash_hash_algo);
  secured_message_context->dhe_key_size    = spdm_get_dhe_pub_key_size (secured_message_context->dhe_named_group);
  secured_message_context->aead_key_size   = spdm_get_aead_key_size (secured_message_context->aead_cipher_suite);
  secured_message_context->aead_iv_size    = spdm_get_aead_iv_size (secured_message_context->aead_cipher_suite);
  secured_message_context->aead_block_size = spdm_get_aead_block_size (secured_message_context->aead_cipher_suite);
  secured_message_context->aead_tag_size   = spdm_get_aead_tag_size (secured_message_context->aead_cipher_suite);
}
...
*/
return_status
spdm_secured_message_export_master_secret (
  IN     void                         *spdm_secured_message_context,
     OUT void                         *export_master_secret,
  ...
  )
{
  ...
  *export_master_secret_size = secured_message_context->hash_size;
  copy_mem (export_master_secret, secured_message_context->handshake_secret.export_master_secret, secured_message_context->hash_size);
  return RETURN_SUCCESS;
}
...
*/
return_status
spdm_secured_message_export_session_keys (
  IN     void                         *spdm_secured_message_context,
  ...
  )
{
  ...
  secured_message_context = spdm_secured_message_context;
  struct_size = sizeof(spdm_secure_session_keys_struct_t) + (secured_message_context->aead_key_size + secured_message_context->aead_iv_size + sizeof(uint64)) * 2;
  ...
  session_keys_struct->aead_key_size = (uint32)secured_message_context->aead_key_size;
  session_keys_struct->aead_iv_size = (uint32)secured_message_context->aead_iv_size;

  ptr = (void *)(session_keys_struct + 1);
  copy_mem (ptr, secured_message_context->application_secret.request_data_encryption_key, secured_message_context->aead_key_size);
  ptr += secured_message_context->aead_key_size;
  copy_mem (ptr, secured_message_context->application_secret.request_data_salt, secured_message_context->aead_iv_size);
  ptr += secured_message_context->aead_iv_size;
  copy_mem (ptr, &secured_message_context->application_secret.request_data_sequence_number, sizeof(uint64));
  ptr += sizeof(uint64);
  copy_mem (ptr, secured_message_context->application_secret.response_data_encryption_key, secured_message_context->aead_key_size);
  ptr += secured_message_context->aead_key_size;
  copy_mem (ptr, secured_message_context->application_secret.response_data_salt, secured_message_context->aead_iv_size);
  ptr += secured_message_context->aead_iv_size;
  copy_mem (ptr, &secured_message_context->application_secret.response_data_sequence_number, sizeof(uint64));
  ptr += sizeof(uint64);
  return RETURN_SUCCESS;
}
...
*/
return_status
spdm_secured_message_import_session_keys (
  IN     void                         *spdm_secured_message_context,
  IN     void                         *SessionKeys,
  ...
  )
{
  ...
  session_keys_struct = SessionKeys;
  if ((session_keys_struct->version != SPDM_SECURE_SESSION_KEYS_STRUCT_VERSION) ||
      (session_keys_struct->aead_key_size != secured_message_context->aead_key_size) ||
      (session_keys_struct->aead_iv_size != secured_message_context->aead_iv_size) ) {
    return RETURN_INVALID_PARAMETER;
  }

  ptr = (void *)(session_keys_struct + 1);
  copy_mem (secured_message_context->application_secret.request_data_encryption_key, ptr, secured_message_context->aead_key_size);
  ptr += secured_message_context->aead_key_size;
  copy_mem (secured_message_context->application_secret.request_data_salt, ptr, secured_message_context->aead_iv_size);
  ptr += secured_message_context->aead_iv_size;
  copy_mem (&secured_message_context->application_secret.request_data_sequence_number, ptr, sizeof(uint64));
  ptr += sizeof(uint64);
  copy_mem (secured_message_context->application_secret.response_data_encryption_key, ptr, secured_message_context->aead_key_size);
  ptr += secured_message_context->aead_key_size;
  copy_mem (secured_message_context->application_secret.response_data_salt, ptr, secured_message_context->aead_iv_size);
  ptr += secured_message_context->aead_iv_size;
  copy_mem (&secured_message_context->application_secret.response_data_sequence_number, ptr, sizeof(uint64));
  ptr += sizeof(uint64);
  return RETURN_SUCCESS;
}
  ...
  )
{
  ...
  secured_message_context = spdm_secured_message_context;
  copy_mem (last_spdm_error, &secured_message_context->last_spdm_error, sizeof(spdm_error_struct_t));
}
  ...
  )
{
  ...
  secured_message_context = spdm_secured_message_context;
  copy_mem (&secured_message_context->last_spdm_error, last_spdm_error, sizeof(spdm_error_struct_t));
}
