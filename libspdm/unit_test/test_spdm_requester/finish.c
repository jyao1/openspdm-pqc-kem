  ...
  )
{
  ...
  copy_mem (secured_message_context->handshake_secret.response_finished_key, key, secured_message_context->hash_size);
}
  ...
  )
{
  ...
  case 0x2:
  {
    ...
    copy_mem (&m_local_buffer[m_local_buffer_size], spdm_response, sizeof(spdm_finish_response_t));
...
void test_spdm_requester_finish_case2(void **state) {
  ...
  assert_int_equal (spdm_secured_message_get_session_state (spdm_context->session_info[0].secured_message_context), SPDM_SESSION_STATE_ESTABLISHED);
  free(data);
}
...
int spdm_requester_finish_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_requester_finish_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
