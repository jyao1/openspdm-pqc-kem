...
uintn
spdm_test_get_psk_exchange_request_size (
  ...
  )
{
  ...
  case 0x2:
    ...
    message_size = spdm_test_get_psk_exchange_request_size (spdm_context, (uint8 *)request + header_size, request_size - header_size);
  ...
  )
{
  ...
  case 0x2:
  {
    ...
    spdm_bin_concat (BIN_STR_2_LABEL, sizeof(BIN_STR_2_LABEL) - 1, hash_data, (uint16)hash_size, hash_size, bin_str2, &bin_str2_size);
    zero_mem (m_local_psk_hint, 32);
    copy_mem (&m_local_psk_hint[0], TEST_PSK_HINT_STRING, sizeof(TEST_PSK_HINT_STRING));
    spdm_psk_handshake_secret_hkdf_expand (m_use_hash_algo, m_local_psk_hint, sizeof(TEST_PSK_HINT_STRING), bin_str2, bin_str2_size, response_handshake_secret, hash_size);
  ...
  case 0x8:
  {
    ...
    spdm_response.extend_error_data.request_code = SPDM_PSK_EXCHANGE;
...
int spdm_requester_psk_exchange_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_requester_psk_exchange_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
