...
void test_spdm_responder_psk_finish_case1(void **state) {
  ...
  m_spdm_psk_finish_request1_size = sizeof(spdm_psk_finish_request_t) + hmac_size;
  response_size = sizeof(response);
  status = spdm_get_response_psk_finish (spdm_context, m_spdm_psk_finish_request1_size, &m_spdm_psk_finish_request1, &response_size, response);
...
void test_spdm_responder_psk_finish_case2(void **state) {
  ...
  spdm_hmac_all (m_use_hash_algo, get_managed_buffer(&th_curr), get_managed_buffer_size(&th_curr), request_finished_key, hash_size, ptr);
  response_size = sizeof(response);
  status = spdm_get_response_psk_finish (spdm_context, m_spdm_psk_finish_request2_size, &m_spdm_psk_finish_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
int spdm_responder_psk_finish_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_psk_finish_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
