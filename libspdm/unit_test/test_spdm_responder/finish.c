  ...
  )
{
  ...
  copy_mem (secured_message_context->handshake_secret.request_finished_key, key, secured_message_context->hash_size);
}
...
void test_spdm_responder_finish_case1(void **state) {
  ...
  spdm_secured_message_set_request_finished_key (session_info->secured_message_context, m_dummy_buffer, hash_size);
  ...
  append_managed_buffer (&th_curr, (uint8 *)&m_spdm_finish_request1, sizeof(spdm_finish_request_t));
  ...
  status = spdm_get_response_finish (spdm_context, m_spdm_finish_request1_size, &m_spdm_finish_request1, &response_size, response);
...
void test_spdm_responder_finish_case2(void **state) {
  ...
  append_managed_buffer (&th_curr, cert_buffer_hash, hash_size);
  // session_transcript.message_k is 0 
  append_managed_buffer (&th_curr, (uint8 *)&m_spdm_finish_request2, sizeof(spdm_finish_request_t));
  set_mem (request_finished_key, MAX_HASH_SIZE, (uint8)(0xFF));
  spdm_hmac_all (m_use_hash_algo, get_managed_buffer(&th_curr), get_managed_buffer_size(&th_curr), request_finished_key, hash_size, ptr);
  response_size = sizeof(response);
  status = spdm_get_response_finish (spdm_context, m_spdm_finish_request2_size, &m_spdm_finish_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
int spdm_responder_finish_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_finish_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
