...
To view the certificate, use "openssl x509 -in xxx.cert.der -inform DER -text"
...
openssl req -nodes -x509 -days 3650 -newkey ec:param.pem -keyout ca.key -out ca.cert -sha512 -subj "/CN=intel test ECP521 CA"
openssl pkey -in ca.key -outform der -out ca.key.der
openssl req -nodes -newkey ec:param.pem -keyout inter.key -out inter.req -sha512 -batch -subj "/CN=intel test ECP521 intermediate cert"
openssl req -nodes -newkey ec:param.pem -keyout end_requester.key -out end_requester.req -sha512 -batch -subj "/CN=intel test ECP521 requseter cert"
openssl req -nodes -newkey ec:param.pem -keyout end_responder.key -out end_responder.req -sha512 -batch -subj "/CN=intel test ECP521 responder cert"
...
==== PQC Hybrid Cert Chain ===============

# NOTE: openssl is the new built one with PQC support.
# NOTE: -config openssl.cnf is the one in openssl/apps dir.
# NOTE: -extfile openssl.cnf is the one in libspdm/unit_test/sample_key dir.

export HYBRID_ALGO=dilithium2
export HYBRID_ALGO=p256_dilithium2
export HYBRID_ALGO=rsa3072_dilithium2
export HYBRID_ALGO=dilithium3
export HYBRID_ALGO=p384_dilithium3
export HYBRID_ALGO=dilithium5
export HYBRID_ALGO=p521_dilithium5
export HYBRID_ALGO=dilithium2_aes
export HYBRID_ALGO=p256_dilithium2_aes
export HYBRID_ALGO=rsa3072_dilithium2_aes
export HYBRID_ALGO=dilithium3_aes
export HYBRID_ALGO=p384_dilithium3_aes
export HYBRID_ALGO=dilithium5_aes
export HYBRID_ALGO=p521_dilithium5_aes

export HYBRID_ALGO=falcon512
export HYBRID_ALGO=p256_falcon512
export HYBRID_ALGO=rsa3072_falcon512
export HYBRID_ALGO=falcon1024
export HYBRID_ALGO=p521_falcon1024

export HYBRID_ALGO=picnicl1full
export HYBRID_ALGO=p256_picnicl1full
export HYBRID_ALGO=rsa3072_picnicl1full
export HYBRID_ALGO=picnic3l1
export HYBRID_ALGO=p256_picnic3l1
export HYBRID_ALGO=rsa3072_picnic3l1

export HYBRID_ALGO=rainbowIclassic
export HYBRID_ALGO=p256_rainbowIclassic
export HYBRID_ALGO=rsa3072_rainbowIclassic
export HYBRID_ALGO=rainbowVclassic
export HYBRID_ALGO=p521_rainbowVclassic

export HYBRID_ALGO=sphincsharaka128frobust
export HYBRID_ALGO=p256_sphincsharaka128frobust
export HYBRID_ALGO=rsa3072_sphincsharaka128frobust
export HYBRID_ALGO=sphincssha256128frobust
export HYBRID_ALGO=p256_sphincssha256128frobust
export HYBRID_ALGO=rsa3072_sphincssha256128frobust
export HYBRID_ALGO=sphincsshake256128frobust
export HYBRID_ALGO=p256_sphincsshake256128frobust
export HYBRID_ALGO=rsa3072_sphincsshake256128frobust

# echo below to gen_algo.sh
mkdir $HYBRID_ALGO
cd $HYBRID_ALGO
../openssl req -nodes -x509 -days 3650 -newkey $HYBRID_ALGO -keyout ca.key -out ca.cert -subj "/CN=intel test $HYBRID_ALGO CA" -config ../openssl.cnf
../openssl req -nodes -newkey $HYBRID_ALGO -keyout inter.key -out inter.req -batch -subj "/CN=intel test $HYBRID_ALGO intermediate cert" -config ../openssl.cnf
../openssl req -nodes -newkey $HYBRID_ALGO -keyout end_requester.key -out end_requester.req -batch -subj "/CN=intel test $HYBRID_ALGO requseter cert" -config ../openssl.cnf
../openssl req -nodes -newkey $HYBRID_ALGO -keyout end_responder.key -out end_responder.req -batch -subj "/CN=intel test $HYBRID_ALGO responder cert" -config ../openssl.cnf
../openssl x509 -req -in inter.req -out inter.cert -CA ca.cert -CAkey ca.key -days 3650 -set_serial 1 -extensions v3_inter -extfile ../../openssl.cnf
../openssl x509 -req -in end_requester.req -out end_requester.cert -CA inter.cert -CAkey inter.key -days 365 -set_serial 2 -extensions v3_end -extfile ../../openssl.cnf
../openssl x509 -req -in end_responder.req -out end_responder.cert -CA inter.cert -CAkey inter.key -days 365 -set_serial 3 -extensions v3_end -extfile ../../openssl.cnf
../openssl asn1parse -in ca.cert -out ca.cert.der
../openssl asn1parse -in inter.cert -out inter.cert.der
../openssl asn1parse -in end_requester.cert -out end_requester.cert.der
../openssl asn1parse -in end_responder.cert -out end_responder.cert.der
