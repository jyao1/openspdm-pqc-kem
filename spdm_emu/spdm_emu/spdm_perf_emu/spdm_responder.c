...
void                              *m_server_spdm_context;

extern uint32 m_server_command;
extern uintn  m_server_receive_buffer_size;
extern uint8  m_server_receive_buffer[MAX_SPDM_MESSAGE_BUFFER_SIZE];
...
return_status
spdm_server_device_send_message (
  IN     void                                   *spdm_context,
  IN     uintn                                  request_size,
  IN     void                                   *request,
  IN     uint64                                 timeout
...
return_status
spdm_server_device_receive_message (
  IN     void                                   *spdm_context,
  IN OUT uintn                                  *response_size,
  ...
  )
{
  ...
  m_server_receive_buffer_size = sizeof(m_server_receive_buffer);
  result = receive_platform_data (0, &m_server_command, m_server_receive_buffer, &m_server_receive_buffer_size);
  if (!result) {
  ...
  if (m_server_command == SOCKET_SPDM_COMMAND_NORMAL) {
    //
  ...
  if (*response_size < m_server_receive_buffer_size) {
    *response_size = m_server_receive_buffer_size;
    return RETURN_BUFFER_TOO_SMALL;
  }
  *response_size = m_server_receive_buffer_size;
  copy_mem (response, m_server_receive_buffer, m_server_receive_buffer_size);
  return RETURN_SUCCESS;
}
  ...
  )
{
  ...
  m_server_spdm_context = (void *)malloc (spdm_get_context_size());
  if (m_server_spdm_context == NULL) {
    return NULL;
  }
  spdm_context = m_server_spdm_context;
  spdm_init_context (spdm_context);
  spdm_register_device_io_func (spdm_context, spdm_server_device_send_message, spdm_server_device_receive_message);
  ...
  spdm_register_session_state_callback_func (spdm_context, spdm_server_session_state_callback);
  spdm_register_connection_state_callback_func (spdm_context, spdm_server_connection_state_callback);

  if (m_load_state_file_name != NULL) {
    // Invoke callback to provision the rest
    spdm_server_connection_state_callback (spdm_context, SPDM_CONNECTION_STATE_NEGOTIATED);
  }

  return m_server_spdm_context;
}
  ...
  case SPDM_SESSION_STATE_NOT_STARTED :
    ...
    if (m_save_state_file_name != NULL) {
      ...
      *(uint32 *)parameter.additional_data = session_id;
      ...
      spdm_get_data (spdm_context, SPDM_DATA_SESSION_END_SESSION_ATTRIBUTES, &parameter, &data8, &data_size);
