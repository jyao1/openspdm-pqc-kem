  ...
  )
{
  ...
  if (m_use_transport_layer == SOCKET_TRANSPORT_TYPE_MCTP) {
    ...
    status = spdm_send_receive_data (spdm_context, &session_id, TRUE, &mSecureSessionRequest, sizeof(mSecureSessionRequest), &app_response, &app_response_size);
    ASSERT_RETURN_ERROR(status);
  ...
  )
{
  ...
  if ((m_exe_session & EXE_SESSION_KEY_UPDATE) != 0) {
    ...
    case SPDM_KEY_UPDATE_ACTION_ALL:
      status = spdm_key_update (spdm_context, session_id, FALSE);
      if (RETURN_ERROR(status)) {
        printf ("spdm_key_update - %x\n", (uint32)status);
      }
      break;
    ...
    case SPDM_KEY_UPDATE_ACTION_RESPONDER:
      ...
      if (!result) {
        ...
        status = spdm_send_receive_encap_request (spdm_context, &session_id);
        if (RETURN_ERROR(status)) {
  ...
  if ((m_exe_session & EXE_SESSION_NO_END) == 0) {
    status = spdm_stop_session (spdm_context, session_id, m_end_session_attributes);
    if (RETURN_ERROR(status)) {
