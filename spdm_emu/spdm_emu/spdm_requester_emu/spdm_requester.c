  ...
  )
{
  ...
  result = send_platform_data (m_socket, SOCKET_SPDM_COMMAND_NORMAL, request, (uint32)request_size);
  if (!result) {
    printf ("send_platform_data Error - %x\n",
#ifdef _MSC_VER
      WSAGetLastError()
  ...
  )
{
  ...
  result = receive_platform_data (m_socket, &command, response, response_size);
  if (!result) {
    printf ("receive_platform_data Error - %x\n",
#ifdef _MSC_VER
      WSAGetLastError()
