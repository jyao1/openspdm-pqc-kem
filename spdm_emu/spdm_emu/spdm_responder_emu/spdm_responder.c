  ...
  )
{
  ...
  result = send_platform_data (m_server_socket, SOCKET_SPDM_COMMAND_NORMAL, request, (uint32)request_size);
  if (!result) {
  ...
  )
{
  ...
  result = receive_platform_data (m_server_socket, &m_command, m_receive_buffer, &m_receive_buffer_size);
  if (!result) {
