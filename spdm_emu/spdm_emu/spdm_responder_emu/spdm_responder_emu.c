...
boolean
create_socket(
  IN  uint16              port_number,
  IN  SOCKET              *listen_socket
  )
{
  ...
  *listen_socket = socket(PF_INET, SOCK_STREAM, 0);
  ...
  res = bind(*listen_socket, (struct sockaddr*) &my_address, sizeof(my_address));
  if(res == SOCKET_ERROR) {
  ...
  )
{
  ...
  while (TRUE) {
    ...
    case SOCKET_SPDM_COMMAND_OOB_ENCAP_KEY_UPDATE:
      ...
      result = send_platform_data (socket, SOCKET_SPDM_COMMAND_OOB_ENCAP_KEY_UPDATE, NULL, 0);
      if (!result) {
        printf ("send_platform_data Error - %x\n",
#ifdef _MSC_VER
          WSAGetLastError()
    ...
    case SOCKET_SPDM_COMMAND_SHUTDOWN:
      result = send_platform_data (socket, SOCKET_SPDM_COMMAND_SHUTDOWN, NULL, 0);
      if (!result) {
        printf ("send_platform_data Error - %x\n",
#ifdef _MSC_VER
          WSAGetLastError()
    ...
    case SOCKET_SPDM_COMMAND_CONTINUE:
      result = send_platform_data (socket, SOCKET_SPDM_COMMAND_CONTINUE, NULL, 0);
      if (!result) {
        printf ("send_platform_data Error - %x\n",
#ifdef _MSC_VER
          WSAGetLastError()
    ...
    case SOCKET_SPDM_COMMAND_NORMAL:
      if (m_use_transport_layer == SOCKET_TRANSPORT_TYPE_PCI_DOE) {
        ...
        ASSERT (m_receive_buffer_size == sizeof(doe_discovery_request_mine_t));
    ...
    default:
      ...
      result = send_platform_data (socket, SOCKET_SPDM_COMMAND_UNKOWN, NULL, 0);
      if (!result) {
        printf ("send_platform_data Error - %x\n",
#ifdef _MSC_VER
          WSAGetLastError()
  ...
  )
{
  ...
  do {
    ...
    m_server_socket = accept(listen_socket, (struct sockaddr*) &peer_address, (socklen_t *)&length);
